# CareerForge Day 7 - The Engineering Defense Board

Product: CareerForge Capability Defense

Role: Engineering Judgment Candidate

Core lesson:

> Capability is not what you completed. Capability is what you can defend.

## Why Day 7 Exists

Day 1 tested debugging.

Day 2 tested systems reasoning.

Day 3 tested readiness verification.

Day 4 tested ambiguity reduction.

Day 5 tested AI trust judgment.

Day 6 tested prioritization under pressure.

Day 7 tests whether you can defend your engineering judgment with evidence.

## What This Is

This is an engineering judgment defense.

It is not:

- a final presentation
- a mock interview
- a motivational closing
- a participation ceremony
- a fake certification moment

You are defending actual work from Days 1-6.

## Student Materials

Use:

- [Student Challenge Brief](./docs/student-challenge-brief.md)
- [Final Defense Dossier Template](./final-defense-dossier-template.md)
- [Engineering Judgment Snapshot](./engineering-judgment-snapshot-template.md)
- [Capability Scorecard Template](./capability-scorecard-template.md)
- [Growth Prescription Template](./growth-prescription-template.md)
- [Final Outcome Tier Record Template](./final-outcome-tier-record-template.md)

## Provided To Solve The Case

- Student challenge brief
- Final Defense Dossier template
- Engineering Judgment Snapshot template
- Your Day 1-Day 6 submissions and evidence
- Live defense board questioning format

## Outcome Artifacts You Get

- Final Defense Dossier
- Engineering Judgment Snapshot
- Live Defense Board Viva
- Capability Scorecard
- Growth Prescription
- Final Outcome Tier

## Final Submission Pack

Submit:

1. Final Defense Dossier
2. Engineering Judgment Snapshot
3. Capability Scorecard
4. Growth Prescription
5. Final Outcome Tier Record
6. Live Defense Board Viva

Optional if founder requests it:

- 3-minute final defense video

## Viva Rule

You will not know which days will be challenged.

Evaluator may choose any 2 days from Day 1-Day 6.

## Fairness Rule

Simple English is acceptable.

Fancy language is not rewarded.

Clear thinking, evidence, and honest ownership matter.

## Outcome Tiers

- Strong Pass
- Pass
- Revision Required
- Incomplete

There is no universal completion praise. Your outcome depends on evidence and defense.
